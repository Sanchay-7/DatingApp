// components/onboarding/Step2_Details.jsx

import React from 'react';

const Step2_Details = ({ formData, updateFormData }) => {

    // Initialize state for the 'Show Gender' toggle inside this component
    // We don't need to save this to formData, as it's a simple UI setting here.
    ...
    const handleDateChange = (name, value) => {
        // In a real application, you'd combine these into a single Date object, 
        // but for simplicity, we'll store them separately for now.
        updateFormData({
            ...formData,
            birthday: {
                ...formData.birthday,
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 max-w-6xl w-full">
            ...
            <section className="col-span-1 space-y-8">
                ...
                {/* 2. Birthday (Simulated DD/MM/YYYY dropdowns) */}
                <div className="space-y-2">
                    ...
                    <div className="flex space-x-4">
                        {['month', 'day', 'year'].map((unit) => (
                            <input
                                key={unit}
                                type="text" // Using text for simplicity, could be <select>
                                name={unit}
                ...
                {/* 4. Show Gender Toggle (NOW FUNCTIONAL) */}
                <div className="flex items-center justify-between pt-4">
                    <span className="text-gray-400">Show my gender on my profile</span>
                    {/* Toggle switch with click handler and state logic */}
                    <div

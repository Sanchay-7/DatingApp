...
const Step2_Details = ({ formData, updateFormData, ageValidationError }) => {
  ...
  const buttonClass = (value) =>
    `px-4 py-2 rounded-full font-semibold transition-colors border ${
          ...
          <div className="p-4 bg-white rounded-xl border border-gray-200">
            <div className="flex gap-2 mb-4">
              {formData.photos.slice(0, 3).map((_, i) => (
                <div
                  key={i}
                  className="w-1/3 aspect-square bg-gray-100 rounded-md grid place-items-center text-xs text-gray-600"
                >
                  P{i + 1}
                </div>
              ))}
